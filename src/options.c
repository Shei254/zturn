...
static int on_help(zturn_options_t *vopts, const char *arg);
static int on_version(zturn_options_t *vopts, const char *arg);

static int on_file(zturn_options_t *vopts, const char *arg) {
	...
	if (zturn_options_from_file(file, vopts) != 0) {
		goto error;
	}
...
error:
	...
	zturn_options_destroy(vopts);
	exit(EXIT_FAILURE);
}

static int on_log(zturn_options_t *vopts, const char *arg) {
	if (*arg == '\0')
		return -1;
...
static int on_log_level(zturn_options_t *vopts, const char *arg) {
...
static int on_daemon(zturn_options_t *vopts, const char *arg) {
	(void)arg;
	vopts->daemon = true;
	return 0;
}

static int on_port(zturn_options_t *vopts, const char *arg) {
	int p = atoi(arg);
	if (p <= 0)
		return -1;
...
static int on_range(zturn_options_t *vopts, const char *arg) {
...
static int on_bind(zturn_options_t *vopts, const char *arg) {
	if (*arg == '\0')
		return -1;
...
static int on_external(zturn_options_t *vopts, const char *arg) {
	if (*arg == '\0')
		return -1;
...
static int on_credentials(zturn_options_t *vopts, const char *arg) {
...
static int on_quota(zturn_options_t *vopts, const char *arg) {
	int n = atoi(arg);
	if (n <= 0)
		return -1;
...
static int on_max(zturn_options_t *vopts, const char *arg) {
	int n = atoi(arg);
	if (n <= 0)
		return -1;
...
static int on_stun_only(zturn_options_t *vopts, const char *arg) {
	(void)arg;
...
typedef struct zturn_option_entry {
	char short_name;
	const char *long_name;
	const char *arg_name;
	const char *description;
	int (*callback)(zturn_options_t *zturn_options, const char *value);
} zturn_option_entry_t;

#define zturn_OPTIONS_COUNT 14
#define HELP_DESCRIPTION_OFFSET 24

static const zturn_option_entry_t zturn_options_map[zturn_OPTIONS_COUNT] = {
...
static int on_help(zturn_options_t *vopts, const char *arg) {
	(void)vopts;
	(void)arg;
	printf("Usage: %s [options]\n\n", program_name ? program_name : "zturn");

	for (int i = 0; i < zturn_OPTIONS_COUNT; ++i) {
		const zturn_option_entry_t *entry = zturn_options_map + i;
...
static int on_version(zturn_options_t *vopts, const char *arg) {
	...
	const char *version = zturn_VERSION;
	printf("zturn version %s\n", version);
...
int zturn_options_from_arg(int argc, char *argv[], zturn_options_t *vopts) {
	...
	char short_options[zturn_OPTIONS_COUNT * 2 + 2];
	...
	*so++ = ':';
	...
	struct option long_options[zturn_OPTIONS_COUNT + 1];
